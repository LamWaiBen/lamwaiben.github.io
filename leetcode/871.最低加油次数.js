...
var minRefuelStops = function(target, startFuel, stations) {
    ...
    while(target > startFuel && i >= 0 && i < stations.length) {
        if(startFuel >= stations[i][0]) {
            arr.push(stations[i][1])
            arr.sort((a, b) => a - b)
            i++
        } 
        
        if(i < stations.length) {
            while(arr.length && startFuel < stations[i][0]) {
                startFuel += arr.pop()
                result += 1
            }
            if(!arr.length && startFuel < stations[i][0]) {
                break
            }
        } else {
            while(arr.length && startFuel < target) {
                startFuel += arr.pop()
                result += 1
            }
            break
    ...
    return target > startFuel ? -1 : result
};
// @lc code=end

// var target = 100, startFuel = 10, stations = [[10,60],[20,30],[30,30],[60,40]]
// var target = 100, startFuel = 1, stations = [[10,100]]
