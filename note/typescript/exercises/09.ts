...
type PromisifyOldFuncDef<T> = (callback: (response: ApiResponse<T>) => void) => void

function promisify<T>(func: PromisifyOldFuncDef<T>): ()=> Promise<T> {
    return () => {
...
// function promisify<T>(func: ((response: T) => void) ): ()=> Promise<T> {
//     return function(){
