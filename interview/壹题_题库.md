...
- 2. vuxe中mutation不能使用异步操作, 可以在actions中使用, 通过commit(mutation)实现异步
...
3. 客户端确认数字证书有效后, 生成一个随机数3(Premaster secret)并使用数字证书中的公钥对其加密, 随后发给服务端
4. 服务端使用私钥, 解密客户端发来的随机数3
5. 客户端和服务端根据约定的加密方法, 使用前面的三个随机数, 生成对话密钥(session key), 用来对称家接下来的整个对话过程
